  ...
  return (
    <div className="relative w-[76px] h-[76px] flex-none">
      <svg width="76" height="76" viewBox="0 0 76 76">
        ...
        <circle cx="38" cy="38" r={r} fill="none" stroke="var(--text-primary)" strokeWidth="7" strokeLinecap="round" strokeDasharray={`${done} ${c}`} transform="rotate(-90 38 38)" />
      </svg>
  ...
  return (
    <div className="mx-auto max-w-[1280px] w-full transition-all duration-200">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-8">
        <div>
          <h1 className="text-[24px] font-disp font-semibold tracking-tight text-[var(--text-primary)]">
            {greeting}, {owner}
          </h1>
      ...
      {!onboarded && (
        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[12px] p-5 mb-6 flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <span className="w-9 h-9 rounded-[10px] bg-[var(--surface-2)] border border-[var(--border)] text-[var(--text-primary)] grid place-items-center flex-none">
      ...
      {!fullySetUp && (
        <div className="grid gap-6 sm:grid-cols-2 mb-6">
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[12px] p-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <ul className="flex flex-col gap-1.5 min-w-0">
                {checklist.map((c) => (
                  <li key={c.label}>
                    <Link href={c.href} className={`flex items-center gap-2 text-[13px] hover:underline ${c.done ? "text-muted line-through" : "text-[var(--text-primary)]"}`}>
                      <span className={`w-[18px] h-[18px] rounded-full flex items-center justify-center flex-none border ${c.done ? "bg-[var(--text-primary)] border-[var(--text-primary)] text-white" : "border-[var(--border)] text-transparent bg-[var(--surface)]"}`}>
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-[12px] p-6 flex flex-col justify-between gap-4">
            <div className="flex items-center gap-3">
              {businessLogo ? (
                <span className="w-10 h-10 rounded-[10px] overflow-hidden flex-none border border-[var(--border)] bg-[var(--surface)] p-1 flex items-center justify-center">
                  <img src={businessLogo} alt={displayName} className="w-full h-full object-cover rounded-[8px]" />
                </span>
              ) : (
                <span className="w-10 h-10 rounded-[10px] bg-[var(--surface-2)] border border-[var(--border)] text-muted flex items-center justify-center font-semibold text-[14px] flex-none">
                  {hasBusinessName ? initials(businessName) : "?"}
                </span>
              )}
              <div className="min-w-0">
                <h3 className="font-disp font-semibold text-[var(--text-primary)] truncate text-[14px]">{displayName}</h3>
                <p className="text-[12px] text-muted line-clamp-1">{description}</p>
              </div>
            </div>
