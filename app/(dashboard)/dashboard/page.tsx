...
import { users, conversations, messages, whatsappConnections, settings } from "@/lib/db/schema";
import { eq, inArray } from "drizzle-orm";
import { createSeed, agoStr, initials, type Product, type Service } from "@/lib/demo";
...
function StatCard({ icon, k, v, s, tone }: { icon: string; k: string; v: string; s: string; tone?: "alert" | "bad" }) {
  return (
    <div
      className={`bg-white border rounded-[16px] p-4 relative overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-[0_14px_30px_-14px_rgba(20,154,91,.35)] ${
        tone === "alert" ? "border-[#EFD9A8] bg-[#FFFCF5]" : tone === "bad" ? "border-[#F0C4C4] bg-[#FFF7F7]" : "border-[#E4EDE5]"
      }`}
    >
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-bold uppercase tracking-[.07em] text-[#8B9B8F]">{k}</span>
        <span
          className={`w-8 h-8 rounded-[10px] flex items-center justify-center flex-none ${
            tone === "bad" ? "bg-red-bg text-red" : tone === "alert" ? "bg-amb-bg text-amber-600" : "bg-grn-bg text-grn-d"
          }`}
        >
      ...
      <div className="font-mono text-[24px] font-bold mt-2 tracking-[-.02em] text-[#17291E]">{v}</div>
      <div className="text-[11.5px] text-[#8B9B8F] mt-0.5">{s}</div>
    </div>
  );
}
    ...
    return (
      <div key={i} className="flex-1 flex flex-col items-center gap-1 min-w-0">
        <div className="w-full flex items-end justify-center" style={{ height: 120 }}>
          <div
            className={`w-[78%] max-w-[26px] rounded-t-[5px] transition-all hover:opacity-100 ${today ? "bg-grn shadow-[0_6px_16px_-6px_rgba(20,154,91,.6)]" : "bg-[#CFE9D8] opacity-80 hover:bg-grn"}`}
            style={{ height: `${bh}%` }}
            title={`${v} messages · ${d.toLocaleDateString("en-GB", { weekday: "short", day: "numeric" })}`}
          />
        </div>
        <span className="font-mono text-[8.5px] text-[#8B9B8F] whitespace-nowrap hidden sm:block">{label}</span>
      </div>
    );
  });
...
function Donut({ a, b }: { a: number; b: number }) {
  ...
  return (
    <div className="flex items-center gap-5">
      <svg width="116" height="116" viewBox="0 0 116 116" className="flex-none">
        <circle cx="58" cy="58" r={r} fill="none" stroke="#E4EDE5" strokeWidth="15" />
        <circle cx="58" cy="58" r={r} fill="none" stroke="#149A5B" strokeWidth="15" strokeDasharray={`${c * pa} ${c}`} strokeLinecap="round" transform="rotate(-90 58 58)" />
        <circle cx="58" cy="58" r={r} fill="none" stroke="#E8A222" strokeWidth="15" strokeDasharray={`${c * (1 - pa)} ${c}`} strokeDashoffset={-c * pa} transform="rotate(-90 58 58)" />
      ...
      <div className="text-[12.5px] flex flex-col gap-2">
        <span className="flex items-center gap-2"><i className="dot g" />AI resolved <b className="font-mono">{a}</b></span>
        <span className="flex items-center gap-2"><i className="dot a" />Handed off <b className="font-mono">{b}</b></span>
        <span className="text-[11.5px] text-[#8B9B8F]">{pct}% fully automated</span>
      </div>
    </div>
  );
}
...
export default async function DashboardPage() {
  ...
  const description = bizSettings?.desc || (data[2] as string) || (hasBusinessName ? "Your AI WhatsApp agent is standing by." : "Add your business name in Business Profile to get started.");
  ...
  try {
    ...
    if (row) {
      const waRes = await db
        ...
        .where(eq(whatsappConnections.userId, row.id))
        .limit(1);
      wa = waRes[0];
    }
  } catch (e) {
  ...
  const catalog = (row?.catalog ?? null) as { products?: Product[]; services?: Service[]; lowStockThreshold?: number } | null;
  const catalogProducts = catalog?.products ?? null;
  const catalogServices = catalog?.services ?? null;
  const products = catalogProducts ?? (seed ? seed.products : null);
  const services = catalogServices ?? (seed ? seed.services : null);
  const threshold = catalogProducts ? (catalog?.lowStockThreshold ?? 3) : seed?.lowStockThreshold ?? 3;
  ...
  const aiResolved = seed ? demo!.aiResolved : 0;
  const handedOff = seed ? demo!.handedOff : 0;
  ...
  return (
    <div className="viewwrap max-w-[1240px] mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        <div>
          <h1 className="text-[26px] font-disp font-bold tracking-tight text-dark">Dashboard</h1>
      ...
      <div className="bg-[#0C2417] rounded-[20px] overflow-hidden mb-5 text-white relative">
        <div
          className="absolute inset-0 opacity-[0.12]"
          style={{
            backgroundImage:
              "radial-gradient(circle at 12% 20%, rgba(83,232,155,.9) 0, transparent 42%), radial-gradient(circle at 88% 0%, rgba(20,154,91,.8) 0, transparent 45%)",
          }}
        />
        <div className="relative p-6 sm:p-7 flex items-center justify-between flex-wrap gap-5">
          <div className="min-w-0 flex items-center gap-4">
            {businessLogo ? (
              ...
              <span className="w-14 h-14 rounded-[16px] bg-lime2 text-[#06170D] flex items-center justify-center font-extrabold font-disp text-[20px] flex-none">
                {hasBusinessName ? initials(businessName) : "?"}
              </span>
            )}
            <div className="min-w-0">
              <div className="text-[11px] font-bold uppercase tracking-[.12em] text-[#8FF0B4] mb-1">{greeting}, {owner}</div>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <Link href="/dashboard/agent" className="inline-flex items-center gap-2 px-4 py-2.5 rounded-[10px] bg-grn text-white font-semibold text-sm hover:bg-grn-d transition-colors">
      ...
      <div className="statgrid">
        <StatCard icon="total" k="Total messages" v={total.toLocaleString()} s={seed ? "all time" : "all time · via WhatsApp"} />
        <StatCard icon="today" k="Messages today" v={today.toLocaleString()} s="▲ since midnight" />
        <StatCard icon="active" k="Active conversations" v={String(active)} s={waiting ? waiting + " waiting for you" : "in your inbox"} tone={waiting ? "alert" : undefined} />
        <StatCard icon="products" k="Total products" v={String(productsN)} s={servicesN ? servicesN + " services" : seed ? "in catalog" : "add your catalog"} />
        <StatCard icon="stock" k="Items in stock" v={String(stock)} s={productsN ? "units across catalog" : "no stock tracked yet"} />
        <StatCard icon="out" k="Out of stock" v={String(outN)} s={lowN ? lowN + " running low" : productsN ? "all stocked" : "—"} tone={outN ? "bad" : undefined} />
        <StatCard icon="ai" k="Resolved by AI" v={aiResolved.toLocaleString()} s={seed ? "this month" : "after agent goes live"} />
        <StatCard icon="human" k="Handed to human" v={handedOff.toLocaleString()} s={seed ? "this month" : "after agent goes live"} />
      </div>
