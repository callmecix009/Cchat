...
export default function () {
  ...
  // Duplicate delivery (idempotency) — same payload again
  if (Math.random() < 0.3) {
    r = http.post(`${BASE}/api/whatsapp/webhook`, payload, { headers });
    check(r, { 'duplicate 200/403': x => [200,403].includes(x.status) });
  }
